...
import uuid
import math
import html  # stdlib; strips HTML entities without requiring bleach
...
def smart_upload_save(request):
    ...
    # [M2] Cap maximum devices per upload
    MAX_DEVICES = 500
    if len(devices_data) > MAX_DEVICES:
        return Response(
            {'error': f'Too many devices in a single upload. Max allowed: {MAX_DEVICES}.'},
        ...
        with transaction.atomic():
            for index, data in enumerate(devices_data):
                try:
                    ...
                    # [M2] Backend validation — wattage range
                    rating = float(data.get('watt_rating') or 0)
                    if not (1 <= rating <= 100_000):
                        raise ValueError(f"Wattage {rating} W is out of valid range (1–100,000 W)")

                    # [M2] Sanitize device name: strip HTML, trim, truncate
                    raw_name = str(data.get('device_type') or 'DEVICE')
                    safe_name = html.escape(raw_name).strip()[:120]
