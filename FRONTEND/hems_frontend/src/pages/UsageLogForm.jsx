...
const UsageLogForm = () => {
    ...
    // [M1] Debounce timer + [M1-follow-up] AbortController for in-flight cancellation
    const debounceRef = useRef(null);
    const abortControllerRef = useRef(null);

    // Calculations are pure-JS (no network call), so the abort controller is
    // primarily used if a future API-backed preview is added, and also cancels
    // the submit POST when the component unmounts mid-flight.
    ...
    }, [selectedDeviceDetails, formData.hoursUsed]);

    // [M1-follow-up] Abort any in-flight request on unmount
    useEffect(() => {
        return () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        // [M1-follow-up] Cancel any previous in-flight submit before starting a new one
        if (abortControllerRef.current) abortControllerRef.current.abort();
        ...
        <div className="max-w-4xl mx-auto p-4 pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                ...
                <motion.form
                    ...
                    className="space-y-6 bg-white/40 backdrop-blur-xl dark:bg-slate-800/50 p-6 rounded-2xl border border-slate-300 dark:border-slate-700/50 backdrop-blur-xl shadow-lg"
                ...
                <motion.div
                    ...
                    className="bg-white/40 backdrop-blur-xl dark:bg-slate-800/50 p-6 rounded-2xl border border-slate-300 dark:border-slate-700/50 backdrop-blur-xl shadow-lg h-full"
                >
