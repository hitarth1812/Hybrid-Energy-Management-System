        ...
        <AnimatePresence>
            {isOpen && (
                <>
                    ...
                    {/* [L3] Positioned wrapper: relative + min-height so the absolute child
                         has an anchor point, works correctly inside iframes too. */}
                    <div style={{ position: 'relative', minHeight: '400px', zIndex: 50 }}
                         className="absolute top-0 left-0 right-0 flex justify-center pt-4 px-4 pointer-events-none">
                        <motion.div
                            initial={{ y: "-100%" }}
                            animate={{ y: 0 }}
                            exit={{ y: "-100%" }}
                            transition={{ type: "spring", damping: 25, stiffness: 200 }}
                            style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: '42rem' }}
